    ...
    if mode != '5':
        ...
        if img_color is not None and img_pillow is not None:
            ...
            try:
                ...
                print("开始提取图片块信息 (用于 Turtle 模式)...") # This print is common for modes 2,3,4
                ...
                print(f"已收集并处理 {len(block_dict)} 个图片块信息 (用于 Turtle 模式)。") # This print is common for modes 2,3,4
                max_block_ix = int(math.ceil(img_width / block_size)) -1
                max_block_iy = int(math.ceil(img_height / block_size)) -1
                print(f"最大块索引: ({max_block_ix}, {max_block_iy}) (用于 Turtle 模式)") # This print is common for modes 2,3,4
        ...
        if mode == '1':
            ...
            for block_entry in blocks_to_draw:
                ...
                if (block_ix, block_iy) in img_to_turtle_map:
                    ...
                    # Move to the top-left position of the block
                    t.penup() # Ensure pen is up before moving
            ...
            while blocks_to_visit_queue and block_count < max_blocks_to_draw_mode3:
                ...
                if (current_ix, current_iy) in img_to_turtle_map:
                    ...
                    turtle_pos_x, turtle_pos_y, block_color, draw_width, draw_height = img_to_turtle_map[(current_ix, current_iy)]
                    # We also need the original block dimensions for angle calculation later if needed
                    # original_block_width = block_dict[(current_ix, current_iy)]['width']
                    # original_block_height = block_dict[(current_ix, current_iy)]['height']
            ...
            # Calculate image dimensions and block size (available from common Pillow loading section)
            # img_width, img_height, block_size are available from the common processing block for modes 2, 3, 4
            # block_dict and img_to_turtle_map are populated from that block
            ...
            # Determine the starting block (center of the image) in block indices
            # Use img_width and img_height from the common section (if mode != 5 block ran)
            # Need to ensure block_size is defined here if the common block for modes 2,3,4 is skipped
            # Block size should be defined regardless of mode if we want consistent indexing logic
            # block_size definition is now outside the if mode != '5' block.
            # img_width, img_height are from img_pillow, which is loaded outside the if mode != '5' block now.
            
            start_ix = int(round(img_width / 2.0 / block_size)) # Use img_width defined earlier
            start_iy = int(round(img_height / 2.0 / block_size)) # Use img_height defined earlier
            ...
            # Check if the starting block is actually in our map of drawable blocks
            if (start_ix, start_iy) in img_to_turtle_map:
                ...
                print(f"起始块中心 Turtle 坐标 (预期绘制起点): ({turtle_pos_x_center:.2f}, {turtle_pos_y_center:.2f})")
                 ...
                 print(f"警告: 计算出的起始块索引 ({start_ix}, {start_iy}) 不存在于 img_to_turtle_map 中。 Square spiral might not start at exact center.")
                 # If the exact center block doesn't exist in the drawable map, the spiral will effectively start from
                 # the first valid block it encounters on its path from the center. This is acceptable.
            ...
            visited_blocks = set() # Re-initialize

            # Add the starting block to visited if it exists in the drawable map
            if (current_ix, current_iy) in img_to_turtle_map:
                visited_blocks.add((current_ix, current_iy))
            else:
                 # If the calculated start block isn't drawable, find a drawable one to start the loop
                 # This is a simple fallback to ensure the loop can start. It might not be the 'best' spiral start.
                 if img_to_turtle_map:
                      current_ix, current_iy = next(iter(img_to_turtle_map)) # Get the first drawable block's index
                      visited_blocks.add((current_ix, current_iy))
                      print(f"使用第一个可绘制块 ({current_ix}, {current_iy}) 作为方形螺旋起点。")
                 else:
                      print("错误: 没有可绘制的块用于模式 4 方形螺旋。")
                      max_blocks_to_draw = 0 # Set limit to 0 to skip the loop


            block_count = 0 # Re-initialize
            max_blocks_to_draw = len(img_to_turtle_map) # Limit drawing to available drawable blocks

            # Turtle starts at center (0,0) for red dot if mode is not 5, then moves to block position for drawing

            while block_count < max_blocks_to_draw:
                # *** Draw the current block before calculating the next move ***
                # Check if the current block is drawable and hasn't been drawn yet
                if (current_ix, current_iy) in img_to_turtle_map and (current_ix, current_iy) not in drawn_blocks:
                    ...
                    # if block_count == 1: # Removed for less verbose output
                    ...
                    if block_count == max_blocks_to_draw: # Check if we've drawn all drawable blocks
                ...
                # Check if the next step changes segment and if the first block of the potential new segment exists and is drawable/unvisited
                steps_taken_in_dir += 1
                change_segment = (steps_taken_in_dir == steps_in_current_segment)

                # move_to_next = False # This variable is not strictly necessary with the current logic flow
                ...
                if change_segment:
                    ...
                    temp_step_length = step_length
                    if (segment_count + 1) % 2 == 0: # Increase step length after every two segments
                         temp_step_length += 1

                    temp_next_ix, temp_next_iy = current_ix, current_iy # Start from current to calculate first step of next segment
                    ...
                    # Check if this first block of the next segment exists in the drawable map AND has not been visited
                    if (temp_next_ix, temp_next_iy) in img_to_turtle_map and (temp_next_ix, temp_next_iy) not in visited_blocks:
                        ...
                        # move_to_next = True # Not needed
                    else:
                         # If the next segment start is invalid or visited, try moving one more step in the current direction
                         # if that block exists and hasn't been visited AND is drawable.

                         # Check if the single step in the *original* direction leads to a valid, unvisited, drawable block
                         ...
                         if (single_step_next_ix, single_step_next_iy) in img_to_turtle_map and (single_step_next_ix, single_step_next_iy) not in visited_blocks:
                             ...
                             # move_to_next = True # Not needed
                              ...
                              # The spiral path is blocked or all available blocks are visited/drawn.
                              # The loop will eventually finish when block_count reaches max_blocks_to_draw.
                              # Stay in the current block for this iteration, and the loop condition will handle termination.
                              pass

                else: # Not changing segment, just attempt to move one step in the current direction
                     # Check if the next block in the current direction exists in the drawable map AND hasn't been visited
                     if (next_ix, next_iy) in img_to_turtle_map and (next_ix, next_iy) not in visited_blocks:
                          ...
                          # If the next block in the current direction doesn't exist, is visited, or not drawable, stop trying to move in this direction
                          # This means the spiral path is broken or all available blocks are visited/drawn.
                          # The loop will eventually finish when block_count reaches max_blocks_to_draw.
                          # Stay in the current block for this iteration, and the loop condition will handle termination.
                          pass

                # If no valid move was made in this iteration, the current_ix, current_iy remains unchanged.
                # The loop will continue trying from the same block in the next iteration,
                # until block_count reaches max_blocks_to_draw, at which point it breaks.
                # A more sophisticated approach might try to find the nearest unvisited drawable block if stuck,
                # but for a simple spiral traversal, being stuck means the path is broken or complete within the drawable area.
