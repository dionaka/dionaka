...
while True:
    ...
    print("7. 背景移除") # Add background removal option
    print("6. 退出") # Add exit option
    print("请选择模式编号 (1, 2, 3, 4, 5, 7 或 6): ")
    mode = input()
    ...
    # Initialize image variables to None/0
    img_color = None
    img_pillow = None
    img_width = 0
    img_height = 0

    # --- Image Loading (for modes 1, 2, 3, 4, 5) ---
    if mode in ['1', '2', '3', '4', '5']:
        image_path = 'your.png' # Assuming the image is still named 'your.png'
        
        try:
            # Try loading with OpenCV first (needed for mode 1)
            ...
            # Try loading with Pillow (needed for modes 2, 3, 4, 5)
            img_pillow = Image.open(image_path)
            img_pillow = img_pillow.convert('RGB') # Ensure RGB format
            img_width, img_height = img_pillow.size
            print(f"Pillow 成功加载图片 {image_path}，尺寸: {img_width}x{img_height}。")
            
        except FileNotFoundError:
            print(f"错误: 无法找到图片 {image_path}。请确保文件存在。")
            # Set variables to None/0 if not found
            img_color = None
            img_pillow = None
            img_width = 0
            img_height = 0
    ...
    elif mode not in ['1', '2', '3', '4', '5', '6', '7']:
        print("错误：选择的模式无效。")
