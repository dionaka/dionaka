...
class Live2DWidget(QOpenGLWidget):
    ...
    def handle_llm_response(self, response_text):
        ...
        # Trigger model talking animation (using Idle_0 motion as a placeholder)
        if self.model_display_area.model:
            print("Attempting to trigger Tapface_0 motion...") # Update print message
            print(f"Model instance is: {self.model_display_area.model}") # Print model instance

            # --- Attempt to list available motions ---
            try:
                ...
                if motion_group_names:
                    ...
                    print("No motion groups found.")
            except AttributeError:
                 # print("Model does not have GetMotionGroupNames method.") # Avoid spamming
                 print("Could not retrieve motion group names.")
            except Exception as e:
                 print(f"Error listing motion groups: {e}")
            # --- End listing available motions ---

            try:
                # Start the first motion in the "Tapface" group with integer priority (2 for Normal)
                self.model_display_area.model.StartMotion("Tapface", 0, 2) # Use group "Tapface" and index 0
                print("StartMotion called for Tapface, 0 with integer priority 2.")
            except Exception as e:
