...
class Live2DWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        if self.model:
            ...
            # TODO: Adjust model position and scale here if needed
            # live2d models typically have coordinates around -1 to 1 or 0 to 2
            # We need to scale and translate the model's coordinate system to fit within the widget's pixel space
            # For example, to center a model that uses -1 to 1 coordinates and scale it:
            # glTranslate(self.width() / 2, self.height() / 2, 0) # Translate to center of widget
            # glScale(self.width() / 2, self.height() / 2, 1) # Scale model to fit widget (adjust factor as needed)
            # Or, for a model using 0 to 2 coordinates:
            # glTranslate(self.width() / 2 - self.width() * self.model.GetCanvasWidth()/ (2 * self.model.GetCanvasWidth()), self.height() / 2 - self.height() * self.model.GetCanvasHeight() / (2 * self.model.GetCanvasHeight()), 0) # Center based on model canvas size
            # glScale(self.width() / self.model.GetCanvasWidth(), self.height() / self.model.GetCanvasHeight(), 1) # Scale to fit based on model canvas size

            # For now, let's try a simple centering and scaling based on common Live2D model scale (e.g., 2 units wide/tall)
            model_scale = min(self.width(), self.height()) / 2.0 # Scale to fit the smaller dimension
            glTranslate(self.width() / 2.0, self.height() / 2.0, 0.0) # Move origin to widget center
            glScale(model_scale, model_scale, 1.0) # Apply scale
            glTranslate(-1.0, -1.0, 0.0) # Assume model is defined in 0..2 range and move its bottom-left to new origin
            # Adjust the last glTranslate if your model's origin is different (e.g., -1 to 1 range)
            ...
            self.model.Update() # Corrected method name
