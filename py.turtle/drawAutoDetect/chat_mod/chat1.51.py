...
class Live2DWidget(QOpenGLWidget):
    ...
    def handle_llm_response(self, response_text):
        ...
        # Define a list of available animations based on model loading logs
        available_animations = [
            ("Idle", 0),
            ("Tapface", 0), ("Tapface", 1),
            ("Taphair", 0), ("Taphair", 1),
            ("Tapxiongbu", 0), ("Tapxiongbu", 1),
            ("Tapqunzi", 0), ("Tapqunzi", 1),
            ("Tapleg", 0), ("Tapleg", 1), ("Tapleg", 2),
        ]

        # Trigger a random animation from the available list
        if self.model_display_area.model:
            print("Attempting to trigger a random motion...") # Update print message
            print(f"Model instance is: {self.model_display_area.model}") # Print model instance

            # --- Attempt to list available motions (retained for info) ---
            try:
                motion_group_names = self.model_display_area.model.GetMotionGroupNames() # Guess method name
                if motion_group_names:
                    print("Available motion groups (from GetMotionGroupNames):")
                    for group_name in motion_group_names:
                        try:
                            motion_count = self.model_display_area.model.GetMotionCount(group_name) # Guess method name
                            print(f"  - {group_name}: {motion_count} motions")
                        except AttributeError:
                            print(f"  - {group_name}: Could not get motion count")
                            # print(f"Model does not have GetMotionCount method.") # Avoid spamming
                        except Exception as e:
                             print(f"Error getting motion count for group {group_name}: {e}")
                else:
                    print("No motion groups found (from GetMotionGroupNames).")
            except AttributeError:
                 # print("Model does not have GetMotionGroupNames method.") # Avoid spamming
                 print("Could not retrieve motion group names using GetMotionGroupNames.")
            except Exception as e:
                 print(f"Error listing motion groups: {e}")
            # --- End listing available motions ---

            try:
                # Randomly select an animation
                selected_animation = random.choice(available_animations)
                group, index = selected_animation

                # Start the selected motion with integer priority (2 for Normal)
                self.model_display_area.model.StartMotion(group, index, 2) # Use integer 2 for Normal priority
                print(f"StartMotion called for group '{group}', index {index} with integer priority 2.")

            except Exception as e:
                print(f"Failed to trigger model animation: {e}") # Added newline for clarity
        else:
            print("Model instance is None. Cannot trigger animation.")
