...
if config:
    ...
    if confirm == '':
        ...
        # 将加载的配置存储到 current_config_values，以便后续重新输入时使用默认值
        current_config_values = config.copy()
        ...
        # 用户选择重新输入，清空config，但保留 current_config_values 作为默认值源
        print("请重新输入配置：")
        config = None
...
if not config:
    ...
    tts_api_choice = tts_api_choice_input if tts_api_choice_input else default_tts_api_choice.lower()
    ...
    if tts_api_choice == 'vits_simple_api':
        default_vits_speaker_id_str = str(default_vits_speaker_id) if default_vits_speaker_id is not None else ''
...
# 如果是加载的配置且有效，直接使用；如果是新输入的，使用 temp_config_values 中的值
used_llm_base_url = llm_base_url if config else temp_config_values.get('llm_base_url')
used_llm_api_key = llm_api_key if config else temp_config_values.get('llm_api_key')
